...
class FacilityBriefing(Base):
    ...
    `inputs_hash` is why this can be cached at all: it fingerprints the stock
    position the line describes, so the advice expires when that position
    changes rather than only when the clock runs out. Yesterday's "order ORS
    today" must not survive today's delivery.

    `model` records which model wrote it. A row generated by the deterministic
    rules and a row generated by Gemini are not interchangeable, and the screen
    labels them differently.
    """

    __tablename__ = "facility_briefings"
