...
@docs/specs/SPEC_DIGEST.md
@docs/specs/HACKATHON_RULES.md
@docs/specs/federated-health-brief.md
@docs/specs/data-trust-layer.md

## JUDGING
Weights live in docs/specs/HACKATHON_RULES.md → "ADDENDUM: OFFICIAL JUDGING CRITERIA" (25 / 20 / 20 / 20 / 15). Read it before any scope decision; do not restate the weights elsewhere.
...
|---|---|
| FL training, silos, aggregation | docs/specs/masterbuildspec-v3.md §12.2, §27, §28 Phase B; docs/specs/masterbuildspec-FINAL.md §9.2 (hand-rolled baseline) |
| Trust score, anomaly detection, audit queue | v3 §12.6, §9.1, §26; docs/specs/research.md Red Team §D |
...
| Maps / Twilio / SMS / WhatsApp / IVR / voice | v3 §13, §14, §14.3, §15, §17, §28 Phase C; docs/specs/HACKATHON_RULES.md §1 (Maps and Twilio are optional, not compliance) |
...
| Deploy, hosting, infra | docs/specs/HACKATHON_RULES.md §1 **first** (Cloud Run is not mandated), then v3 §28 Phase E, §16 |
| Testing, eval harness, demo rehearsal | v3 §23, §28 Phase D; docs/specs/implementation.md (build order and checkpoints) |

- **KEYS.md**: holds variable names and setup steps only. Never `@`-import it, never quote from it, never copy a value out of it.
- **Which spec is current**: v3 opens "Supersedes v2"; FINAL self-identifies as "v2 (FINAL)", so v3 reads as later. **Not confirmed canonical — ask Aditya before treating either as authoritative**, and read docs/specs/SPEC_DIGEST.md's conflict list first.
...
- **Status lines are stale**: `main` is at `542d8dc`. docs/specs/SPEC_DIGEST.md §5 and v3 §28 report Phase B complete and Phase C/D partly built; that code was deliberately removed. Verify against git before trusting any "done".
...
- **Keys protocol** (full version: docs/specs/HACKATHON_RULES.md §3): before each stage, list every key, credential, project ID, endpoint and URL that stage needs, with exact env var names, and ask for all of them up front. If something unanticipated is needed mid-task, stop that task at that step and ask. Never substitute a placeholder, fake value or silent mock and present it as real. Never mark a stage complete if any part of it leaned on a stand-in — say plainly which part did.
